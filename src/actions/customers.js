...
import CustomerDataService from "../services/customers.service";
...
export const retrieveCustomers = (token) => async (dispatch) => {
    try {
        const res = await CustomerDataService.getAll(token);
        dispatch({
...
export const deleteCustomer = (token, id) => async (dispatch) => {
    try {
        await CustomerDataService.delete(token, id);
